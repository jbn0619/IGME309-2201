...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	*/
	...
	for (int i = 0; i < a_nSubdivisions; i++)
	{
		...
		firstAngleToUse = firstAngleToUse * (3.14 / 180);
		secondAngleToUse = secondAngleToUse * (3.14 / 180);

		// Make the variosu triangle points.
		vector3 perimeterStartingPoint = centerPoint + vector3(cos(firstAngleToUse), sin(firstAngleToUse), 0.0f);
		vector3 perimeterEndingPoint = centerPoint + vector3(cos(secondAngleToUse), sin(secondAngleToUse), 0.0f);

		// Once points are made, make a triangle out of them.
		AddTri(centerPoint, perimeterStartingPoint, perimeterEndingPoint);
	}
